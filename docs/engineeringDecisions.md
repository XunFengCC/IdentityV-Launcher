...
2026-09-25，主干源码把组合 emoji 修复作为默认构建输入：在独立不可变 `r1-emoji2` runtime 上，用 CodeWeavers 26.1 GDI 补丁与 OFL 派生字体替换原 GDI 路径。普通源码构建取得该候选；公开 `v1.0.0-rc.1` tag 和安装包仍固定原 r1。PE DLL 单独校验 AMD64 架构与 SHA-256，字体及两份 OFL 声明随 runner 入包。该改动没有把未解决的语音问题或音频模块并入。
