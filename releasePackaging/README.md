...
`/Applications` 中的已装 App、工作树 `build/` 里的候选和某次 DMG 是三个不同对象；版本号相同也不能共享签名、公证和源码对应结论。本仓迁入独立项目目录后，应从**当前新仓提交**构建并记录其 App、DMG、材料与源码清单的精确哈希。旧私人工作区曾形成通过签名、公证与 Gatekeeper 的 RC1 本地候选；它只作字节/行为对照，不能冒充本仓新构建或已公开发布物。干净首次安装、macOS 15 实机和国际服登录保持仍待验收。
