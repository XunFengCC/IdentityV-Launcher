...
同一机器的隔离安装输入里还有相同 bundle ID 的候选副本，GUI 自动化曾选中该副本：国服进程出现、登录代理跟随就绪并从界面正常结束，**只能计为候选副本的宿主流程验证**，不能写作 `/Applications` 安装版的完整游戏登录/对局验收。麦克风实际输入、国际服登录保持、macOS 15 干净首装和最终隐私/许可审查仍分别需要真实证据；构建、公证及本次部分旧装升级不能替代这些门槛。
