...
随后用 `/Applications` 安装版内嵌 runner 启动国服，风吟确认进入大厅；当时她在游戏内**实际发送语音**后收到“未检测到语音”，游戏保持前台按 Rainy 75 的 F11 时出现 macOS 音量提示，切换窗口时偶有破音。此前 CoreAudio 建流与 F 键 watcher 进入标准键行只能证明中间步骤，不能推翻这些反馈。之后她从**日用安装版 GUI 按钮**再启动国服，同一进程中开盖的内建麦克风可实际录入、合盖仍不能；Apple silicon MacBook 合盖断开内建麦克风的硬件行为解释本次差异。内建键盘 F 区较早曾正常，稍后重测也与 Rainy 一样失效；第二把外接键盘 F1–F9 可进游戏但特殊 F10–F12 单按仍被系统快捷键接管，按键时 watcher 实际记录映射租约失败。停游戏后的同设备集合短时租约探针成功写入与恢复；新诊断候选重新运行时四键盘服务均建映射，风吟确认内建键盘恢复。Rainy 在 Mac 模式下 F1–F9 仍异常，F11/Fn+F11 成为音量减；短暂切到 Windows 模式后同一局 F1/F5 正常，但修饰键物理位置变化。该设备层对照解释部分剩余差异，不能冒充所有 F1–F9 或日用 Mac 模式通过。反复切窗未再出现爆音，但根因未定。这些观察不支持把三项现象统归为身份更名回归，也不能称所有设备已通过。新旧标识、偏好和 helper 行为可由本仓 [`signing/verifyFirstPartyIdentity.py`](../signing/verifyFirstPartyIdentity.py)、[`LegacyPreferenceMigrationSelfTest.swift`](../playerLauncherApp/Tests/LegacyPreferenceMigrationSelfTest.swift) 及 [`idvLoginJob.test.command`](../privilegedHelpers/idvLoginJob.test.command) 复核；它们是源码/合约证据，不代替宿主体验结果。
