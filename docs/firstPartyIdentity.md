...
同一机器的隔离安装输入里还有相同 bundle ID 的候选副本，GUI 自动化曾选中该副本：国服进程出现、登录代理跟随就绪并从界面正常结束，**只能计为候选副本的宿主流程验证**，不能写作 `/Applications` 安装版的完整游戏登录/对局验收。实际语音、国际服登录保持、macOS 15 干净首装和最终隐私/许可审查仍分别需要真实证据；构建、公证及本次部分旧装升级不能替代这些门槛。

随后用 `/Applications` 安装版内嵌 runner 启动国服，风吟实际确认进入大厅；这补足了安装版 runner 到游戏大厅的证据，仍未验证玩家 GUI 按钮的完整路径或对局。她在游戏内**实际发送语音**后收到“未检测到语音”，游戏保持前台按 F11 时出现 macOS 音量提示，切换窗口时偶有破音。此前 CoreAudio 曾建立麦克风流、F 键 watcher 曾报告进入标准键行，均不能据此宣布这两项端到端功能通过；F11 行为也需先与现行设计说明核对，不预设一定由 bundle ID 更名造成。当前尚无足够证据把语音、F 区或切窗音频异常归因到身份、权限或单一补丁，已暂停新增实机测试，待后续针对性复测。新旧标识、偏好和 helper 行为可由本仓 [`signing/verifyFirstPartyIdentity.py`](../signing/verifyFirstPartyIdentity.py)、[`LegacyPreferenceMigrationSelfTest.swift`](../playerLauncherApp/Tests/LegacyPreferenceMigrationSelfTest.swift) 及 [`idvLoginJob.test.command`](../privilegedHelpers/idvLoginJob.test.command) 复核；它们是源码/合约证据，不代替宿主体验结果。
